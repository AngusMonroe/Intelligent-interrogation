...
Showbo.Msg={  
    ...
    alert:function(msg){  
      this.show({buttons:{yes:'确认'},msg:msg,title:'您的建议用药有：'});  
    },  
    ...
    show:function(cfg){  
      ...
      if(cfg.wait){  
        ...
        if(cfg.msg){
          const medicine_list=cfg.msg;
          const list_length=medicine_list.length;
          var medicine='';
          var medicine_url=[
            'http://drugs.medlive.cn/drugref/html/566.shtml',
            'http://drugs.medlive.cn/drugref/html/11262.shtml',
            'http://drugs.medlive.cn/drugref/html/17306.shtml',
            'http://drugs.medlive.cn/drugref/html/14391.shtml',
            'http://drugs.medlive.cn/drugref/html/12245.shtml',
          ];
          var url_index=0;
          this.dvCT.innerHTML+='<div style="height:45px"></div>';
          for(i=0;i<list_length;i++){
            if(medicine_list[i]!=';'){
              medicine+=medicine_list[i];
            }
            else{
              this.dvCT.innerHTML+='<div class="medicine_strand">'+'<a class="medicine_a" target="_blank" href="'+medicine_url[url_index]+'">'+medicine+'</a>'+'<button class="feedback_a" onclick="feedback_alert()" ></button>'+'</div>';  
              medicine='';
              url_index++;
            }
          }
          this.dvCT.innerHTML+='<div class="clear"></div>'; 
        }
    ...
    onResize:function(isResize){  
      ...
      //  Showbo.Msg.lightBox.style.height=Showbo.BodyScale.ty+'px';  
       Showbo.Msg.lightBox.style.width='100%';  
       Showbo.Msg.lightBox.style.height='100%';  
       Showbo.Msg.dvMsgBox.style.top=Math.floor((Showbo.BodyScale.y-Showbo.Msg.dvMsgBox.offsetHeight)/2)+'px';  
