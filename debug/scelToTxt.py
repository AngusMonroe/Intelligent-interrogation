...
#作者：scarecrow
#来源：核心代码来源于 哥本草根微博 本人仅是加工包装核心代码方便他人使用
...
def getFileList(dirPath,suffix="scel"):
    fileList=[]
...
#在运行py文件的时候需要在后面跟一个源路径的参数 例:python scelTotxt.py ./
if __name__ == '__main__':  
    try:
        sourcePath="/users/xujiaxing/downloads/八大菜系菜单列表.scel"
        sourcePath=sys.argv[1]
        print(sourcePath)
        if sourcePath == "":
            print("You dont input source,please input source!")
        else:
            #将要转换的词库添加在这里就可以了  
            ...
            #保存结果    
            f = open('sougou1.txt','w')  
            GTable.sort()
            for count,py,word in GTable:  
                #GTable保存着结果，是一个列表，每个元素是一个元组(词频,拼音,中文词组)，有需要的话可以保存成自己需要个格式  
                f.write( unicode(word).encode('utf-8') )#最终保存文件的编码，可以自给改  我这里只输出了单词
                f.write('\n')  
            f.close()  
        print ("--------END---------")
    except Exception:
